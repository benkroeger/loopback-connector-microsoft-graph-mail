...
function activitiesConnector(settings, dataSource) {
  const connector = {};
  Connector.call(connector, 'microsoft-graph-mail', settings);
  ...
  // create reference for our future httpClient instance
  let httpClient;
  ...
  function connect(callback) {
    httpClient = new OniyiHttpClient({
      defaults: {
    ...
    if (settings.plugins && settings.plugins.credentials) {
      httpClient.useHttpClientPlugin(attachCredentialsPlugin(settings.plugins.credentials));
    }
...
module.exports = activitiesConnector;
